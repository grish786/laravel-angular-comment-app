...
//include_once "scpt/class.ecommerce_c.php";
...
$_POST = array(
    "process" => "authorize",
   "cc_number"=>"4574310480076950",
   "cc_code"=>"107",
   "subtotal"=>"373.75",
   "isApruvd"=>"FALSE",
   "isAutoOrder"=>"Y",
   "dealer_id"=>"2840",
   "shopping_cart_id"=>"2356920",
   "g_php_self"=>"/checkout-d.php",
   "financecheckout_self"=>"financecheckout.php",
   "dealer_location_id"=>"2840",
   "dealer_tax_rate"=>"0.080000",
   "dealer_name"=>"C.O.P.S. Inc",
   "dealer_address"=>"1210 East State Street",
   "dealer_city"=>"Sycamore",
   "dealer_state"=>"IL",
   "dealer_zip_code"=>"60178",
   "dealer_phone_number"=>"(815) 899-2997",
   "dealer_url"=>"shop.copsinc.com",
   "shipping_charge"=>0,
   "shipping_weight"=>4.28,
   "express_shipping_code"=>"",
   "ground_shipping_code"=>"",
   "additional_charge"=>0.00,
   "additional_charge_ids"=>"",
   "tax_charge"=>29.90,
   "order_total"=>403.65,
   "order_total_a"=>403.65,
   "gift_card_enabled"=>"",
   "token_info"=>"2356920b76fc558f89c7d8446054a26c9b64bf6d",
   "nfdn_shipping_enabled"=>"",
   "nfdn_insurance_enabled"=>"",
   "auto_order_enabled"=>"Y",
   "insurance_charge"=>0,  
   "shipping_preference"=>"hold",
   "shipping_rate_preference"=>"express",
   "email_address"=>"kcbishop1994@live.com",
   "phone_number"=>"8152171868",
   "notes"=>"",
   "billing_is_shipping"=>"Y",
   "cc_name"=>"Kristopher Bishop",
   "cc_address"=>"809 Kimberly Dr Apt 4",
   "cc_city"=>"Dekalb",
   "cc_state"=>"IL",
   "cc_zip"=>"60115",
   "cc_exp_month"=>"08",
   ...
   "discount_code"=>"",
   "discount_amount"=>"",
   "subcriber_status"=>"enabled",
   "customer_shipping_address_required"=>"",
   "customer_shipping_address_validated"=>"",
   "customer_shipping_address_type"=>"",
   "ship_name"=>"",
   "ship_address"=>"",
   "ship_city"=>"",
   "ship_state"=>"",
   "ship_zip"=>"",
   "ffl_shipping_address_required"=>"Y",
   "ffl_shipping_address_validated"=>"",
   "ffl_shipping_address_type"=>"",
   "ffl_name"=>"",
   "ffl_address"=>"",
   "ffl_city"=>"",
   "ffl_state"=>"",
   ...
   "ffl_phone"=>"",
   "subscribe"=>"no",
   "order_item_0"=>"152103|1|373.75|handguns",
);
$_SESSION["shopping_cart_id"] =$_POST["shopping_cart_id"] ; 
print_r($_POST);
...
trackLog("ShoppingCartId :" . print_r($shoppingCartId, true));
trackLog("Dealer Id from Shopping cart :" . print_r($dealerId, true));
...
function sendAlertToDealer($code, $dealerID, $consumerEmail, $cc_name, $cc_address, $cc_phone) {
   ...
   switch ($code) {
      ...
      case "007":
         ...
         break;
      case "008":
         $message = "Order total doesn't include live price calculation";
         break;
      default:
   ...
   //$mail->AddAddress("radha@mstsolutions.com");
...
while (!empty($_POST['order_item_' . $i])) {
   ...
   // if ($_POST['subtotal'] == 0 || ($cc_amount < $_POST['subtotal'])) {
   if ($_POST['subtotal'] == 0 || empty($_POST['order_total'])) {
      $returnArray['status'] = "cost_error";
      $returnArray['message'] = "Cart error [008] - please try again.";
      //$returnArray['message'] = $price;
     // sendAlertToDealer('008', $dealerId, $consumerInfo);
      echo json_encode($returnArray);
      exit;
   }
...
if ($gLoggedIn) {
   ...
   $resultSet = executeQuery("update contacts set first_name = ?, last_name = ?, address_1 = ?, address_2 = ?, city = ?, state = ?, zip_code = ?, country_code = ?, " .
           "phone_number = ?, email_address = ?, version = version + 1 where contact_id = ?", $firstName, $lastName, $parameterList['address_1'], $parameterList['address_2'], $parameterList['city'], $parameterList['state'], $parameterList['zip_code'], $parameterList['country_code'], $parameterList['phone_number'], $parameterList['email_address'], $contactId);
   ...
   if ($saveShippingAddress) {
      ...
      $resultSet = executeQuery("select * from addresses where (address_type_id is null or address_type_id in " .
              ...
              "phone_number <=> ? and contact_id = ?", $_POST['ship_name'], $_POST['ship_address'], $_POST['ship_city'], $_POST['ship_state'], $_POST['ship_zip'], $_POST['phone_number'], $contactId);
      if ($row = getNextRow($resultSet)) {
         ...
         $resultSet = executeQuery("insert into addresses (address_id,contact_id,full_name,address_label,address_1,address_2," .
                 "city,state,zip_code,country_code,phone_number,email_address,address_type_id,version) values (null,?,?,'Ship To',?,null,?,?,?,?,?,?,?,1)", $contactId, $_POST['ship_name'], $_POST['ship_address'], $_POST['ship_city'], $_POST['ship_state'], $_POST['ship_zip'], $_POST['ship_country'], $_POST['phone_number'], $parameterList['email_address'], $addressTypeId);
   ...
   if ($saveFFLAddress) {
      ...
      $resultSet = executeQuery("select * from addresses where address_type_id in (select address_type_id from address_types where address_type_code = 'FFL') " .
              ...
              "zip_code <=> ? and phone_number <=> ? and contact_id = ?", $_POST['ffl_name'], $_POST['ffl_address'], $_POST['ffl_city'], $_POST['ffl_state'], $_POST['ffl_zip'], $_POST['ffl_phone'], $contactId);
      if ($row = getNextRow($resultSet)) {
         $parameterList['ffl_address_id'] = $row['address_id'];
      } else {
         $resultSet = executeQuery("insert into addresses (address_id,contact_id,full_name,address_label,address_1,address_2," .
                 "city,state,zip_code,country_code,phone_number,address_type_id,version) values (null,?,?,?,?,null,?,?,?,?,?,?,1)", $contactId, $_POST['ffl_name'], $parameterList['full_name'], $_POST['ffl_address'], $_POST['ffl_city'], $_POST['ffl_state'], $_POST['ffl_zip'], $_POST['ship_country'], $_POST['ffl_phone'], $addressTypeId);
   ...
   $resultSet = executeQuery("insert into contacts (contact_id,dealer_idd,first_name,last_name,address_1,city,state,zip_code,country_code,email_address,phone_number,version) values (" .
           "null,?,?,?,?,?,?,?,?,?,?,1)",$dealerId, $firstName, $lastName, $parameterList['address_1'], $parameterList['city'], $parameterList['state'], $parameterList['zip_code'], $parameterList['country_code'], $parameterList['email_address'], $parameterList['phone_number']);
   $contactId = $resultSet['insert_id'];

  if ($resultSet['sql_error']) {
  trackLog("SQL ERROR:" . print_r($resultSet['sql_error'], true));
  break;
   }
   trackLog("Dealer Id" . print_r($dealerId, true));
   trackLog("Dealer Id" . print_r($contactId, true));
   trackLog("Track Contact Details" . print_r($parameterList, true));
...
$resultSet = executeQuery("select * from addresses where (address_type_id is null or address_type_id in (select address_type_id from address_types where address_type_code <> 'FFL')) " .
        "and full_name <=> ? and address_1 <=> ? and address_2 <=> ? and city <=> ? and state = ? and zip_code <=> ? and phone_number <=> ? and contact_id = ?", $parameterList['full_name'], $parameterList['address_1'], $parameterList['address_2'], $parameterList['city'], $parameterList['state'], $parameterList['zip_code'], $parameterList['phone_number'], $contactId);
if ($row = getNextRow($resultSet)) {
   $billingAddressId = $row['address_id'];
} else {
   $resultSet = executeQuery("insert into addresses (address_id,contact_id,full_name,address_label,address_1,address_2," .
           "city,state,zip_code,country_code,phone_number,address_type_id,version) values (null,?,?,'Bill To',?,?,?,?,?,?,?,?,1)", $contactId, $parameterList['full_name'], $parameterList['address_1'], $parameterList['address_2'], $parameterList['city'], $parameterList['state'], $parameterList['zip_code'], $parameterList['country_code'], $parameterList['phone_number'], $addressTypeId);
...
while (true) {
   ...
   $resultSet = executeQuery("insert into orders (order_id,dealer_id,location_id,order_number,contact_id,order_method_id,ffl_address_id,billing_address_id," .
           ...
           "(null,?,?,?,?,?,?,?,?,?,?,now(),?,?,?,?,?,?,?,?,0,1,?)", $dealerId, $dealerLocationId, $orderNumber, $contactId, $orderMethodId, $parameterList['ffl_address_id'], $billingAddressId, $parameterList['shipping_method_id'], $parameterList['shipping_address_id'], $_SERVER['REMOTE_ADDR'], $parameterList['date_completed'], $parameterList['shipping_charge'], $parameterList['insurance_charge'], $parameterList['tax_charge'], $orderStatusId, $parameterList['notes'], $parameterList['print_notes'], $transactionResult, $_GET['ad-link']);
...
$parameterList['transaction_identifier'] = '20170201101555-1116355-10';
$parameterList['authorization_code']='H41982'; 
   ...
   $resultSet = executeQuery("insert into order_payments (order_payment_id,order_id,payment_date,settlement_date,payment_method_id," .
           "reference_number,amount,transaction_identifier,authorization_code,version) values (null,?,?,?,1,?,?,?,?,1)", $orderId,$nowdate,
           $nowdate,$parameterList['reference_number'], $ccAmount, $parameterList['transaction_identifier'], $parameterList['authorization_code']);

echo "insert into order_payments (order_payment_id,order_id,payment_date,settlement_date,payment_method_id," .
           "reference_number,amount,transaction_identifier,authorization_code,version) values (null,?,?,?,1,?,?,?,?,1)", $orderId,$nowdate,
           $nowdate,$parameterList['reference_number'], $ccAmount, $parameterList['transaction_identifier'], $parameterList['authorization_code'];
